...
cv::Mat manzanas_chicas(cv::Mat mascara);

cv::Mat cambiar_valor(cv::Mat imagen,int valor);

int determinar_valor(int area);

float detectar_angulo(const cv::Mat &imagen,const std::vector<cv::Vec2f> &rho_tita);

cv::Mat rotar(const cv::Mat &imagen,const std::vector<cv::Vec2f> &rho_tita);

int promedio_intensidad_mask(const cv::Mat &imagen,const cv::Mat &mascara);

int porcentaje_llenado(const cv::Mat &llena,const cv::Mat &liquido);
